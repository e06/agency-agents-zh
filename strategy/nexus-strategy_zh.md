...
## The Agency 多智能体编排的完整运营手册

> **NEXUS** 将 The Agency 的独立 AI 专家转变为协同运作的智能网络。这不仅仅是一个提示词集合 —— 它是一套**部署准则**，将 The Agency 转化为任何项目、产品或组织的力量倍增器。
...
1. [战略基础](#1-战略基础)
2. [NEXUS 运营模型](#2-the-nexus-operating-model)
3. [阶段 0 — 智能与发现](#3-phase-0--intelligence--discovery)
...
8. [阶段 5 — 发布与增长](#8-phase-5--launch--growth)
9. [阶段 6 — 运营与演进](#9-phase-6--operate--evolve)
10. [智能体协调矩阵](#10-agent-coordination-matrix)
11. [交接协议](#11-handoff-protocols)
12. [质量门禁](#12-quality-gates)
...
- **何时**他们交接以及交给谁
- **如何**在推进前验证质量
- **为什么**每个智能体存在于流程中（没有乘客）

### 1.2 核心原则

| 原则 | 描述 |
|------|------|
| **管道完整性** | 任何阶段必须通过质量门禁才能推进 |
| **上下文连续性** | 每次交接携带完整上下文 —— 没有智能体从零开始 |
| **并行执行** | 独立工作流并行运行以压缩时间线 |
...
| 部门 | 智能体 | 主要 NEXUS 角色 |
|------|--------|-----------------|
| **工程** | 前端开发者、后端架构师、移动应用构建者、AI 工程师、DevOps 自动化工程师、快速原型师、高级开发者 | 构建、部署和维护所有技术系统 |
| **设计** | UI 设计师、UX 研究员、UX 架构师、品牌守护者、视觉叙事师、趣味注入师、图像提示工程师 | 定义视觉识别、用户体验和品牌一致性 |
| **营销** | 增长黑客、内容创作者、Twitter 互动师、TikTok 策略师、Instagram 策展人、Reddit 社区建设者、应用商店优化师、社交媒体策略师 | 驱动获客、参与度和市场影响力 |
| **产品** | 冲刺优先级排序师、趋势研究员、反馈综合师 | 定义构建什么、何时构建以及为什么构建 |
| **项目管理** | 工作室制作人、项目引导者、工作室运营、实验追踪师、高级项目经理 | 编排时间线、资源和跨职能协调 |
| **测试** | 证据收集者、现实检验师、测试结果分析师、性能基准测试师、API 测试师、工具评估师、工作流优化师 | 通过基于证据的评估验证质量 |
| **支持** | 支持响应者、分析报告师、财务追踪师、基础设施维护者、法务合规检查师、执行摘要生成器 | 维持运营、合规和商业智能 |
| **空间计算** | XR 界面架构师、macOS 空间/Metal 工程师、XR 沉浸开发者、XR 座舱交互专家、visionOS 空间工程师、终端集成专家 | 构建沉浸式和空间计算体验 |
| **专业** | 智能体编排器、数据分析报告师、LSP/索引工程师、销售数据提取智能体、数据整合智能体、报告分发智能体 | 跨领域协调、深度分析和代码智能 |
...
| **NEXUS-Micro** | 5-10 | Bug 修复、内容营销活动、单一交付物 | 1-5 天 |

---

## 3. Phase 0 — Intelligence & Discovery

> **目标**：在投入资源之前了解全局。在问题被验证之前不开始构建。
...
> **目标**：定义我们要构建什么、如何构建以及成功是什么样的 —— 在编写一行代码之前。
...
> **目标**：通过持续的 Dev↔QA 循环实现功能。每个任务在下一个开始前都要验证。这是大部分工作发生的地方。

### 6.1 The Dev↔QA Loop

这是 NEXUS 的核心。智能体编排器管理**逐任务的质量循环**：
...
> **目标**：最终的质量关卡。现实检验师默认判定为"需要改进" —— 你必须用压倒性的证据证明生产就绪。
