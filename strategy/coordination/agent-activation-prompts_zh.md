...
> 用于激活 NEXUS 流水线中任何智能体的现成提示词模板。复制、自定义 `[占位符]`，然后部署。
...
|------|-----------|-----------|
| 启动新项目 | 编排器 — 完整流水线 | — |
| 构建功能 | 编排器 — 开发↔QA 循环 | 开发者 + 证据收集器 |
| 修复 Bug | 后端/前端开发工程师 | API 测试工程师或证据收集器 |
| 运行营销活动 | 内容创作者 | 社交媒体策略师 + 平台智能体 |
| 准备发布 | 参见第 5 阶段 Playbook | 所有营销 + DevOps 智能体 |
| 月度报告 | 执行摘要生成器 | 分析报告器 + 财务追踪器 |
| 事件响应 | 基础设施维护者 | DevOps 自动化工程师 + 相关开发者 |
| 市场研究 | 趋势研究员 | 分析报告器 |
| 合规审计 | 法务合规检查器 | 执行摘要生成器 |
